...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (uLineStyle > 0.5) {
        ...
        if (uLineStyle < 1.5) { lineX *= dashMaskY; lineY *= dashMaskX; }
            ...
            float cy = abs(fract(gridUV.y*dotRepeat)-0.5); float cx2 = abs(fract(gridUV.x*dotRepeat)-0.5);
            float dotMaskY = 1.0-smoothstep(dotWidth, dotWidth+fwidth(gridUV.y*dotRepeat), cy);
            float dotMaskX = 1.0-smoothstep(dotWidth, dotWidth+fwidth(gridUV.x*dotRepeat), cx2);
    ...
    if (uLineStyle > 0.5) {
        ...
        if (uLineStyle < 1.5) { lineX2 *= dashMaskY2; lineY2 *= dashMaskX2; }
            ...
            float cy2=abs(fract(gridUV2.y*dotRepeat2)-0.5); float cx22=abs(fract(gridUV2.x*dotRepeat2)-0.5);
            float dotMaskY2=1.0-smoothstep(dotWidth2,dotWidth2+fwidth(gridUV2.y*dotRepeat2),cy2);
            float dotMaskX2=1.0-smoothstep(dotWidth2,dotWidth2+fwidth(gridUV2.x*dotRepeat2),cx22);
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof window !== 'undefined' && (window as Window & typeof globalThis & { DeviceOrientationEvent?: { requestPermission?: () => Promise<string> } }).DeviceOrientationEvent && (DeviceOrientationEvent as unknown as { requestPermission?: () => Promise<string> }).requestPermission) {
  ...
  // Face-api model loading - only when webcam is enabled
  useEffect(() => {
    if (!enableWebcam) return; // guard: skip CDN fetch when webcam is off
    let canceled = false;
    const load = async () => {
      try {
        await Promise.all([
  ...
  // Webcam + face tracking - only when enabled and models ready
  useEffect(() => {
    let stop = false;
    let lastDetect = 0;
    const start = async () => {
...
function smoothDampVec2(current: THREE.Vector2, target: THREE.Vector2, currentVelocity: THREE.Vector2, smoothTime: number, maxSpeed: number, deltaTime: number): THREE.Vector2 {
  ...
  if (originalTo.clone().sub(current).dot(out.clone().sub(originalTo)) > 0) { out.copy(originalTo); currentVelocity.set(0, 0); }
  return out;
}
...
function medianPush(buf: number[], v: number, maxLen: number) {
  buf.push(v); if (buf.length > maxLen) buf.shift();
}
function median(buf: number[]) {
  if (buf.length === 0) return 0;
  const a = [...buf].sort((x, y) => x - y); const mid = Math.floor(a.length / 2);
