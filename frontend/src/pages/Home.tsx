    ...
    <>
      ...
      <section className="relative py-24 px-6 md:px-20 max-w-6xl mx-auto" id="courses" style={{ zIndex: 1 }}>
        ...
        <div className="flex flex-col items-center">
          {/* Level 1 */}
          <div
            className="reveal w-full max-w-2xl glass-card p-8 relative overflow-hidden transition-all duration-300"
            style={{ borderTop: '2.5px solid var(--electric)' }}
          ...
          >
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {['ChatGPT', 'Google Gemini', 'Canva AI', 'DALL-E', 'Microsoft Copilot', 'Quillbot', 'Gamma.app'].map((t) => (
                <span key={t} className="px-2.5 py-0.5 rounded-full text-xs" style={{ background: 'rgba(255,255,255,.05)', border: '1px solid var(--border)', color: 'var(--muted)' }}>{t}</span>
              ))}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 mb-6">
              {['Build presentations in minutes', 'Write emails & content with AI', 'Generate images & posters', 'Research any topic instantly', 'Automate simple daily tasks', 'Start your AI journey confidently'].map((o) => (
                <div key={o} className="flex items-start gap-2 text-xs" style={{ color: 'var(--muted)' }}>
                  <span style={{ color: 'var(--electric)', flexShrink: 0 }}>✓</span>{o}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-4xl">
            {[
              ...
              >
                ...
                <div className="flex flex-wrap gap-3 mb-3 text-xs" style={{ color: 'var(--muted)' }}>
                  {c.meta.map((m) => <span key={m}>📅 {m}</span>)}
                </div>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {c.tools.map((t) => <span key={t} className="px-2 py-0.5 rounded-full text-xs" style={{ background: 'rgba(255,255,255,.05)', border: '1px solid var(--border)', color: 'var(--muted)' }}>{t}</span>)}
                </div>
